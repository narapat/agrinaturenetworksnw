...
import { MapPin, ArrowRight, ShieldCheck, Search } from 'lucide-react';

interface FarmsClientViewProps {
  initialFarms: Farm[];
  farmProductCounts?: Record<string, number>;
}

export default function FarmsClientView({
  initialFarms,
  farmProductCounts = {},
}: FarmsClientViewProps) {
  const { getTextClass } = useFontSize();
  const [allFarms, setAllFarms] = useState<Farm[]>(initialFarms);
  const [selectedDistrict, setSelectedDistrict] = useState<string>('ทั้งหมด');
  const [searchQuery, setSearchQuery] = useState<string>('');

  useEffect(() => {
    const handleDataUpdate = () => {
      setAllFarms(dataService.getPublicFarms());
    };
  ...
  // กรองในหน่วยความจำทันใจ
  const filteredFarms = React.useMemo(() => {
    let list = allFarms;
    if (selectedDistrict !== 'ทั้งหมด') {
      list = list.filter((f) => f.district === selectedDistrict);
    }
    if (searchQuery.trim()) {
    ...
    return list;
  }, [allFarms, selectedDistrict, searchQuery]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 text-xs sm:text-sm text-stone-500 px-1">
        <p>
          พบทั้งหมด <b className="text-stone-800">{filteredFarms.length}</b> แปลง {selectedDistrict !== 'ทั้งหมด' ? `ใน อ.${selectedDistrict}` : ''}
        </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredFarms.map((farm) => {
          const productCount = farmProductCounts[farm.id] ?? (
            typeof window !== 'undefined' ? dataService.getProductsByFarmId(farm.id).length : 0
          );
