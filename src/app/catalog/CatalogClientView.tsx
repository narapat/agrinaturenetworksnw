...
function CatalogClientContent({ initialProducts, initialSkuGroups }: CatalogClientViewProps) {
  const { getTextClass } = useFontSize();
  const searchParams = useSearchParams();
  const initialSku = searchParams.get('sku') || '';

  const [allProducts, setAllProducts] = useState<Product[]>(initialProducts);
  const [allSkuGroups, setAllSkuGroups] = useState<SKUGroup[]>(initialSkuGroups);
  ...
  // ฟังเหตุการณ์เมื่อมีการแก้ไขข้อมูลสินค้าในเครื่อง
  useEffect(() => {
    const handleDataUpdate = () => {
      setAllProducts(dataService.getPublicProducts());
      setAllSkuGroups(dataService.getGroupedSKUs());
    };
  ...
  // กรองสินค้าในหน่วยความจำทันที (Instant Filtering without Network Delay)
  const filteredProducts = React.useMemo(() => {
    let prods = allProducts;

    if (selectedDistrict !== 'ทั้งหมด') {
      prods = prods.filter((p) => p.district === selectedDistrict);
    }
    ...
    if (selectedStatus !== 'ทั้งหมด') {
      prods = prods.filter((p) => p.status === selectedStatus);
    }
    ...
    return prods;
  }, [allProducts, selectedDistrict, selectedCategory, selectedStatus, selectedSkuTag, searchQuery]);

  // กรองกลุ่ม SKU ตามหมวดหมู่และอำเภอที่เลือก
  const filteredSkuGroups = React.useMemo(() => {
    let groups = allSkuGroups;
    if (selectedCategory !== 'ทั้งหมด') {
      groups = groups.filter((g) => g.category === selectedCategory);
    }
    if (selectedDistrict !== 'ทั้งหมด') {
      groups = groups.filter((g) => g.districts.includes(selectedDistrict));
    }
    return groups;
  }, [allSkuGroups, selectedCategory, selectedDistrict]);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pt-2 border-t border-stone-100">
          ...
          <div className="grid grid-cols-2 gap-2 w-full sm:w-auto">
            ...
            >
              ...
              <span>สินค้า ({filteredProducts.length})</span>
            </button>
          </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold text-stone-900 flex items-center gap-2">
              ...
              <span>กลุ่มผลผลิตยอดนิยมในจังหวัดนครสวรรค์ ({filteredSkuGroups.length} กลุ่ม)</span>
            </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5 sm:gap-6">
            {filteredSkuGroups.map((group) => {
              const groupProducts = filteredProducts.filter((p) => p.skuTagId === group.skuTagId);
          ...
          <div className="flex items-center justify-between gap-2">
            <h2 className="text-base sm:text-xl font-bold text-stone-900 flex items-center gap-2 truncate">
              ...
              <span className="truncate">
                {selectedSkuTag 
                  ? `หมวด: ${allSkuGroups.find(g => g.skuTagId === selectedSkuTag)?.name || selectedSkuTag}` 
                  : `ผลผลิตทั้งหมด (${filteredProducts.length})`}
              </span>
            </h2>
            {selectedSkuTag && (
              <button
...
export default function CatalogClientView(props: CatalogClientViewProps) {
  return (
    <Suspense
      fallback={
